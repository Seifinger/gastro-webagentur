...
| Video mobil | ✅ Hochformat-Video (MP4 + WebM) für 9 Küchen (ohne Chinesisch, Vietnamesisch, Asiatisch); Ausschnitt wie das Hochformat-Poster (`--fokus-video-mobil`); Startfrist 10 s, Antippen bei gesperrtem Autoplay, `?video=diagnose` | 25.09.2026 |
...
3. **Hero-Medium**
   ...
   - Das Video lädt erst nach `load`, und nur ohne `prefers-reduced-motion`, ohne
     `Save-Data`/`prefers-reduced-data` und ohne eindeutig langsames Netz (2G; „3g“
     nur zusammen mit unter 1,5 Mbit/s – Chrome meldet „3g“ schon bei hoher Latenz).
   - Poster = erstes Videobild, deshalb ist auch ein später Start nahtlos. Läuft es
     nicht binnen 10 Sekunden an, wird das Laden abgebrochen und das Poster bleibt.
   - Autoplay gesperrt (iPhone-Stromsparmodus u. ä.): nichts wird geladen, das
     Video startet beim ersten Antippen der Seite.
   - Diagnose am Handy: Seite mit `?video=diagnose` öffnen – unten steht, was passiert.
