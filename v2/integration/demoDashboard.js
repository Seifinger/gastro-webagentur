...
//   POST /intern/v2/demo/:slug/vorschau          Vorschau bauen (v2/output/leads/<slug>/)
//   POST /intern/v2/demo/:slug/veroeffentlichen  Speichern-und-Veröffentlichen-Ablauf starten
...
import { starteVeroeffentlichung, laeuftGerade } from "../../src/veroeffentlichung.js";
...
const AKTION = /^\/intern\/v2\/demo\/([^/]+)\/(speichern|vorschau|veroeffentlichen)$/;
...
export function demoAnsicht(slug) {
  ...
  try {
    ...
    medien = medienUebersicht(loeseMedien({ slug, gestaltung, ds, leadEdits: loadLeadEdits(slug), offline: true, konzeptVon: `beispiel-${e.kueche}` }));
  } catch {
    medien = [];
  }
...
export async function demoHandler(req, res, pathname) {
  ...
  try {
    ...
    if (aktion[2] === "speichern") {
      ...
      // Erst speichern (übersteht einen Reload), dann den Ablauf starten.
      if (Object.keys(aenderung).length) speichereDemoEinstellungen(slug, aenderung);
      const d = demoAnsicht(slug);
      if (!d) throw new Error("Zu diesem Entwurf gibt es keinen Lead.");
      if (d.hindernisse.length) throw new Error(d.hindernisse.join(" "));
      const { buildId } = starteVeroeffentlichung(slug);
      sende(res, 202, { ok: true, buildId, demo: demoAnsicht(slug) });
    }
  } catch (fehler) {
