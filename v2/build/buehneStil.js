...
(function () {
  ...
  //    reduzierten Daten oder eindeutig langsamem Netz. Das Poster bleibt stehen,
  //    bis der erste Frame läuft. Poster = erstes Videobild, deshalb ist auch ein
  //    später Start nahtlos; die Startfrist verhindert nur endloses Laden.
  //    Mit ?video=diagnose zeigt die Seite unten, was passiert (für Tests am Handy).
  var video = document.querySelector(".buehne-video");
  var protokoll = function () {};
  if (/[?&]video=diagnose(&|$)/.test(window.location.search)) {
    var tafel = document.createElement("pre");
    tafel.setAttribute("style", "position:fixed;left:8px;right:8px;bottom:8px;z-index:9999;max-height:45vh;overflow:auto;margin:0;padding:8px;background:rgba(0,0,0,.88);color:#fff;font:12px/1.4 monospace;white-space:pre-wrap");
    document.body.appendChild(tafel);
    var beginn = Date.now();
    protokoll = function (text) { tafel.textContent += ((Date.now() - beginn) / 1000).toFixed(1) + " s  " + text + "\\n"; };
  }
  var stand = function (wert) { if (video) video.setAttribute("data-stand", wert); protokoll("Stand: " + wert); };
  var netz = navigator.connection || {};
  var typ = netz.effectiveType || "";
  var datenSparen = window.matchMedia && window.matchMedia("(prefers-reduced-data: reduce)").matches;
  // "3g" meldet Chrome schon bei hoher Latenz im Mobilfunk – langsam nur, wenn
  // auch die gemessene Bandbreite gering ist. 2G ist immer zu langsam.
  var langsam = Boolean(netz.saveData) || datenSparen || /(^|-)2g$/.test(typ) || (typ === "3g" && !(netz.downlink >= 1.5));
  var STARTFRIST_MS = 10000;
  protokoll("Breite " + window.innerWidth + " px, Bewegung reduziert: " + (reduziert ? "ja" : "nein") + ", Netz: " + (typ || "keine Auskunft") + (netz.downlink ? " / " + netz.downlink + " Mbit/s" : "") + (netz.saveData ? ", Datensparen" : ""));
  if (video && reduziert) stand("reduziert");
  else if (video && langsam) stand("langsam");
  else if (video) {
    var starte = function () {
      ...
      if (schmal && video.hasAttribute("data-nur-breit")) { stand("nur-breit"); return; }
      ...
      protokoll("Quelle: " + quelle + " (MP4: " + (h264 || "nein") + ", WebM: " + (vp9 || "nein") + ")");
      var gestartet = false;
      var abgebrochen = false;
      var wartetAufTippen = false;
      var frist = 0;
      // Läuft das Video in der Frist nicht an: Laden abbrechen, Standbild bleibt.
      ...
      var fristStarten = function () { window.clearTimeout(frist); frist = window.setTimeout(abbrechen, STARTFRIST_MS); };
      ...
      video.addEventListener("error", function () { video.classList.remove("laeuft"); if (!abgebrochen) stand("fehler " + (video.error ? video.error.code : "")); });
      ["loadstart", "loadedmetadata", "canplay", "waiting", "stalled"].forEach(function (name) { video.addEventListener(name, function () { protokoll(name); }); });
      // Stummschaltung auch als Eigenschaft – Voraussetzung für Autoplay auf iOS.
      video.muted = true;
      ...
      var spielen = function () {
        if (abgebrochen || (einmal && video.ended)) return;
        var p = video.play();
        if (!p || !p.catch) return;
        p.catch(function (fehler) {
          protokoll("play() abgelehnt: " + fehler.name);
          if (fehler.name !== "NotAllowedError" || gestartet || abgebrochen || wartetAufTippen) return;
          // Autoplay gesperrt (iPhone im Stromsparmodus, „Video-Vorschauen
          // automatisch abspielen“ aus): nichts wird geladen, das Standbild
          // bleibt – beim ersten Antippen startet das Video.
          window.clearTimeout(frist);
          wartetAufTippen = true;
          stand("blockiert");
          var nachTippen = function () {
            document.removeEventListener("touchend", nachTippen, true);
            document.removeEventListener("click", nachTippen, true);
            wartetAufTippen = false;
            fristStarten();
            spielen();
          };
          document.addEventListener("touchend", nachTippen, true);
          document.addEventListener("click", nachTippen, true);
        });
      };
      video.src = quelle;
      fristStarten();
      ...
      if ("IntersectionObserver" in window) {
        new IntersectionObserver(function (eintraege) {
          if (eintraege[0].isIntersecting) { if (!wartetAufTippen) spielen(); } else video.pause();
        }).observe(video);
      }
    };
